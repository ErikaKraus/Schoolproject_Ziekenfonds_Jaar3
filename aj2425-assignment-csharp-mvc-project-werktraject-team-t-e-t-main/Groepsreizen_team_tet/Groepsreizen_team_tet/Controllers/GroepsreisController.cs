...
public class GroepsreisController : Controller
{
    ...
    #region Kandidaat stellen als monitor uitvoeren : POST
    [HttpPost]
    [ValidateAntiForgeryToken]
    ...
    #region Kandidaat stellen als hoofdmonitor uitvoeren : POST
    [HttpPost]
    [ValidateAntiForgeryToken]
...
""";
    ...
    #region Ingeschreven reizen
    public async Task<IActionResult> IngeschrevenReizen()
    {
        // Haal de ingelogde gebruiker op
        var gebruikerId = int.Parse(User.FindFirstValue(ClaimTypes.NameIdentifier));

        // Haal alle groepsreizen op waar de gebruiker kinderen voor heeft ingeschreven
        ...
        var kinderenReizen = groepsreizen
        .SelectMany(g => g.Deelnemers)
        .Where(d => d.Kind.Ouder.Id == gebruikerId)
        .GroupBy(d => d.Kind)
        .Select(group => new KindReizenViewModel
        {
            KindId = group.Key.Id,
            KindNaam = group.Key.Naam,
            KindVoornaam = group.Key.Voornaam,
            ToekomstigeReizen = group
                .Where(d => !d.Groepsreis.IsGeannuleerd && d.Groepsreis.Begindatum > DateTime.Now)
                .Select(d => new GroepsreisDetailViewModel
                {
                    Id = d.Groepsreis.Id,
                    BestemmingNaam = d.Groepsreis.Bestemming.Naam,
                    Begindatum = d.Groepsreis.Begindatum,
                    Einddatum = d.Groepsreis.Einddatum
                })
                .ToList(),
            AfgelopenReizen = group
                .Where(d => !d.Groepsreis.IsGeannuleerd && d.Groepsreis.Einddatum <= DateTime.Now)
                .Select(d => new GroepsreisDetailViewModel
                {
                    Id = d.Groepsreis.Id,
                    BestemmingNaam = d.Groepsreis.Bestemming.Naam,
        ...
        var viewModel = new GroepsreisIngeschrevenReizenViewModel
        {
            KinderenReizen = kinderenReizen
        };
